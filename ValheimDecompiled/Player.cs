...
public class Player : Humanoid
{
    ...
    private void UpdatePlacement(bool takeInput, float dt)
    {
        ...
        if (this.InPlaceMode())
        {
            ...
            if (Hud.IsPieceSelectionVisible())
                return;
            ItemDrop.ItemData rightItem = this.GetRightItem();
    ...
    {
        foreach (ItemDrop.ItemData equipedtem in this.m_inventory.GetEquipedtems())
        {
    ...
    {
        if (item.m_shared.m_itemType != ItemDrop.ItemData.ItemType.Trophie || this.m_trophies.Contains(item.m_dropPrefab.name))
            return;
    ...
    private void OnInventoryChanged()
    {
        if (this.m_isLoading)
            return;
        foreach (ItemDrop.ItemData allItem in this.m_inventory.GetAllItems())
        {
    ...
    {
        if (item.m_shared.m_itemType != ItemDrop.ItemData.ItemType.Consumable || (double)item.m_shared.m_food > 0.0 && !this.CanEat(item, true))
            return false;
